...
class UsuarioModel extends \Com\Daw2\Core\BaseDbModel
{
    ...
    private const NUM_FILAS_PAGINA = 25;
    ...
    private const SELECT_COUNT = "SELECT count(*)
                                    FROM usuario us
                                    JOIN aux_rol ar ON us.id_rol = ar.id_rol
                                    LEFT JOIN aux_countries ac ON us.id_country = ac.id";
    ...
    public function getUsuarioFiltros(array $filtros, int $order, int $numPagina): array
    {
        $sentidoOrder = ($order < 0) ? 'DESC' : 'ASC';
        $order = abs($order);
        $limite = $numPagina * self::NUM_FILAS_PAGINA;
        if (empty($filtros)) {
            $stmt = $this->pdo->query(self::SELECT_FROM . ' ORDER BY ' . self::ORDER_COLUMNS[$order - 1] . " $sentidoOrder" . ' LIMIT ' . $limite-self::NUM_FILAS_PAGINA . ', ' . $limite);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } else {
            $condiciones = $this->getCondiones($filtros);
            $sql = self::SELECT_FROM . ' WHERE ' . implode(' AND ', $condiciones) . ' ORDER BY ' . self::ORDER_COLUMNS[$order - 1] . " $sentidoOrder" . ' LIMIT ' . $limite-self::NUM_FILAS_PAGINA . ', ' . $limite;
    ...
    public function maxPaginas($filtros)
    {
        if (empty($filtros)) {
            $statement = $this->pdo->query(self::SELECT_COUNT);
    ...
    private function getCondiones($filtros)
    {
        $condiciones = [];
